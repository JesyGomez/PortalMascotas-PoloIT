...
const PetCard = ({ pet }) => {
  ...
  const handleAdopt = () => {
    alert(`¡Gracias por querer adoptar a ${nombre}! Te contactaremos pronto.`);
  };

  const handleFavorite = () => {
    alert(`${nombre} añadido/a a tus favoritos ❤️`);
  };

  const handleChat = () => {
    alert(`Abrirías un chat para preguntar por ${nombre}.`);
  };

  const handleBookmark = () => {
    const savedPets = JSON.parse(localStorage.getItem("savedPets")) || [];
    const alreadySaved = savedPets.some((p) => p.nombre === nombre);

    if (!alreadySaved) {
      savedPets.push(pet);
      localStorage.setItem("savedPets", JSON.stringify(savedPets));
      alert(`${nombre} guardado/a para más tarde.`);
    } else {
      alert(`${nombre} ya está guardado/a.`);
    }
  };
    ...
    <Card className="pet-card shadow rounded-4 m-2">
      ...
      >
        ...
        <div className="d-flex align-items-center gap-2">
          <i
            className="bi bi-heart-fill heart-icon-estado"
            style={{
              color: estado === "Adoptado" ? "#E2574C" : "#F4A525",
            }}
          ></i>
      ...
      <Card.Body className="text-center">
        ...
        <div className="d-flex justify-content-around align-items-center gap-3 my-3 flex-wrap">
          ...
          <Button variant="light" className="icon-btn" onClick={handleBookmark}>
